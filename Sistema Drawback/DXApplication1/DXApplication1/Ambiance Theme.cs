...
namespace Ambiance
{
    ...
    public class Ambiance_TabControl : TabControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            G.FillPath(new SolidBrush(Color.Transparent  /*Color.FromArgb(32, 41,50)*/), RoundRectangle.RoundRect(0, 23, Width - 1, Height - 24, 2));
            G.DrawPath(new Pen(Color.Transparent  /*Color.FromArgb(32, 41,50)*/), RoundRectangle.RoundRect(0, 23, Width - 1, Height - 24, 2));
            ...
            for (int ItemIndex = 0; ItemIndex <= TabCount - 1; ItemIndex++)
            {
                ...
                if (ItemIndex == SelectedIndex)
                {
                    ...
                    // Draw header tabs
                    G.DrawPath(new Pen(Color.FromArgb(220, 20, 40)), RoundRectangle.RoundedTopRect(new Rectangle(new Point(ItemBoundsRect.X - 2, ItemBoundsRect.Y - 2), new Size(ItemBoundsRect.Width + 3, ItemBoundsRect.Height)), 7));
                    G.FillPath(new SolidBrush(Color.FromArgb(220, 20, 40)), RoundRectangle.RoundedTopRect(new Rectangle(new Point(ItemBoundsRect.X - 1, ItemBoundsRect.Y - 1), new Size(ItemBoundsRect.Width + 2, ItemBoundsRect.Height)), 7));

                    try
                    {
                        G.DrawString(TabPages[ItemIndex].Text, new Font(Font.Name, Font.Size - 1, FontStyle.Bold), new SolidBrush(Color.White), new Rectangle(GetTabRect(ItemIndex).Location, GetTabRect(ItemIndex).Size), new StringFormat
                        {
                            LineAlignment = StringAlignment.Center,
                            Alignment = StringAlignment.Center
                        });
                        TabPages[ItemIndex].BackColor = Color.Transparent  /*Color.FromArgb(32, 41,50)*/;
                    }
                    catch
                    {
                    }
