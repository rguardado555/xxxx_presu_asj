...
namespace ModuloPresupuesto
{
    public partial class Frm_Etapas : Form
    {
        ...
        void llenargrilla()
        {
            dtg_datos.DataSource = NFunciones.TABLADATOS("SELECT E.IDEMPRESA,E.DESCRIPCION EMPRESA,P.IDETAPA ID,P.DESCRIPCION ETAPA  FROM P_ETAPA P INNER JOIN P_EMPRESA E ON E.IDEMPRESA=P.IDEMPRESA ORDER BY P.IDETAPA ASC");
        ...
        private void btn_nuevo_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                dtid = NFunciones.TABLADATOS("EXEC SP_IDETAPA''");
                this.IsNuevo = true;
                this.IsEditar = false;
                this.Botones();
                this.HabilitarTexbox(true);
        ...
        private void btn_guardar_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                if (this.IsNuevo)
                {
                    ...
                    rpta = NFunciones.ExecuteSQL("INSERT INTO P_ETAPA(IDEMPRESA,IDETAPA,DESCRIPCION)VALUES('"+Convert.ToString(cbo_empresa.EditValue)+"','"+txt_codigo.Text+"','"+txt_descripcion.Text+"')");
                }
                else
                {
                    rpta = NFunciones.ExecuteSQL("UPDATE P_ETAPA SET DESCRIPCION='"+txt_descripcion.Text+"',IDEMPRESA='"+Convert.ToString(cbo_empresa.EditValue)+"' WHERE IDETAPA='"+txt_codigo.Text+"'");
                }
            ...
            if (!this.txt_codigo.Text.Equals(""))
            {
                ...
                try
                {
                    ...
                    if (Opcion == DialogResult.OK)
                    {
                        ...
                        rpta = NFunciones.ExecuteSQL(" DELETE P_ETAPA WHERE IDETAPA='" + txt_codigo.Text + "'");
                        if (rpta.Equals("Ok"))
                        {
