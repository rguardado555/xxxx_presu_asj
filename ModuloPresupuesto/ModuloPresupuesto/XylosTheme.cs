...
public class XylosCombobox : ComboBox
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        checked
        {
            if (enabled)
            {
                using (Pen pen = new Pen(Helpers.ColorFromHex("#202932")))
                {
            ...
            {
                using (Pen pen2 = new Pen(Helpers.ColorFromHex("#f2f2f7")))
                {
                    using (SolidBrush solidBrush2 = new SolidBrush(Helpers.ColorFromHex("#202932")))
                    {
            ...
            if (flag)
            {
                using (Font font3 = new Font("Segoe UI", 9f))
                {
                    using (SolidBrush solidBrush3 = new SolidBrush(Helpers.ColorFromHex("#7C858E")))
                    {
                        ...
                        {
                            using (SolidBrush solidBrush4 = new SolidBrush(Helpers.ColorFromHex("#202932")))
                            {
        ...
        checked
        {
            if (enabled)
            {
                ...
                try
                {
                    ...
                    {
                        using (new Pen(Helpers.ColorFromHex("#202932")))/*#D0D5D9*/
                        {
                            bool flag = (e.State & DrawItemState.Selected) == DrawItemState.Selected;
                            if (flag)
                            {
                                using (new SolidBrush(Color.Transparent))
                                {
